        ...
        execute(message, args, config, Discord, mongoose, GuildModel) {
                if (!args[2]) {
                        ...
                        var req = GuildModel.findOne({ "_id": message.guild.id }, function (err, req) {
                                if (!req) {
                                        try {
                                                const doc = new GuildModel({ "_id": message.guild.id, "guildName": message.guild.name, "modRole": "Moderator", "memberRole": "Member", "mutedRole": "Muted", "banMessage": "", "kickMessage": "", "muteMessage": "", "autoMod": false, bannedWords: false, bannedWordsList: [] });
                                                doc.save();
